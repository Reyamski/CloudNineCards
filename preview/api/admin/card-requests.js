/**
 * /api/admin/card-requests — admin card_requests read endpoint.
 *
 * GET → returns ALL card_requests rows, newest first.
 *
 * Service-role read; mirrors /api/admin/order-items. The card_requests table
 * has RLS ON with no anon policies (docs/card-requests-table.sql), so the
 * admin UI can't read it with the anon key — this endpoint does.
 *
 * No writes — rows are inserted by /api/card-request and are read-only here.
 *
